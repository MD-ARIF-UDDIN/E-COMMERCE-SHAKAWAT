    ...
    <div className="space-y-12 pb-32">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 px-4">
        <div>
           ...
           <h1 className="text-4xl lg:text-5xl font-black text-slate-950 tracking-tighter">Orders</h1>
        ...
        <div className="flex flex-wrap gap-3">
          <button onClick={() => setFilter('')} className={`h-12 px-6 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${filter === '' ? 'bg-slate-950 text-white shadow-premium' : 'bg-slate-50 text-slate-400 border border-slate-100 hover:bg-white'}`}>All Orders</button>
          {STATUSES.map(s => (
            <button 
              key={s} 
              onClick={() => setFilter(s)} 
          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((order, index) => (
              ...
              >
                {/* Order Row */}
                <div
                  className="flex flex-col lg:flex-row lg:items-center gap-8 p-10 cursor-pointer"
                ...
                >
                  ...
                  <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-12">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm font-black text-slate-950 tracking-tight font-mono">{order.orderNumber}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Customer Info</p>
                      <div className="flex flex-col">
                        <p className="text-sm font-black text-slate-950 tracking-tight">{order.customerName || 'Guest'}</p>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex flex-col">
                        <p className="text-sm font-black text-slate-950 tracking-tight">৳{order.totalAmount?.toLocaleString()}</p>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">{order.paymentMethod}</p>
                      </div>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Status</p>
                      <span className={`inline-flex px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border border-current ${STATUS_CONFIG[order.status].bg} ${STATUS_CONFIG[order.status].text}`}>
                        {order.status}
                      </span>
                    </div>
                  </div>
                    ...
                    >
                      <div className="p-12 grid lg:grid-cols-12 gap-12">
                        ...
                        <div className="lg:col-span-7 space-y-8">
                           ...
                           <div className="space-y-3">
                              {order.items?.map((item: any, i: number) => (
                                <div key={i} className="flex justify-between items-center bg-white border border-slate-100 rounded-3xl p-6 shadow-sm group/item hover:border-indigo-200 transition-colors">
                                  <div className="flex items-center gap-4">
                                     <div className="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center text-slate-300 group-hover/item:text-indigo-600 transition-colors">
                                        <Package size={20} />
                                     </div>
                                     <div>
                                        <p className="text-sm font-black text-slate-950 tracking-tight">{item.product?.name || 'Unknown Product'}</p>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Qty: {item.quantity}</p>
                                     </div>
                                  </div>
                                  <span className="text-sm font-black text-slate-950 tracking-tight">৳{(item.price * item.quantity).toLocaleString()}</span>
                                </div>
                              ))}
                           </div>
                        ...
                        <div className="lg:col-span-5 space-y-8">
                           ...
                           <div className="grid grid-cols-2 gap-3">
                              {STATUSES.map(s => (
                                <button
                                  ...
                                  className={`h-14 rounded-2xl text-[10px] font-black uppercase tracking-widest border-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2
                           ...
                           <div className="p-8 bg-slate-950 rounded-[2rem] space-y-6 shadow-premium">
                              <div className="flex items-center gap-3 mb-2">
                                 <Truck size={14} className="text-indigo-400" />
                                 <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest">Logistics / Delivery Company</p>
                              </div>
                              <div className="flex gap-2">
                                <input
                                  ...
                                  className="flex-1 bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-[10px] font-black text-white uppercase tracking-widest focus:bg-white/10 focus:outline-none transition-all placeholder:text-slate-600"
                                />
                                <button
                                  onClick={() => {
                                  ...
                                  className="px-6 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-500 transition-all shadow-lg shadow-indigo-600/20"
                                >
                                  Update
                                </button>
                              </div>

    ...
    <div className="space-y-16 pb-32 max-w-5xl">
      ...
      <div className="px-4">
         <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Security Management</p>
         </div>
         <h1 className="text-4xl lg:text-6xl font-black text-slate-950 tracking-tighter">Blacklist</h1>
         <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-2 max-w-lg">
      ...
      <div className="grid lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-5 space-y-8">
           <div className="bg-slate-950 rounded-[3rem] p-12 text-white relative overflow-hidden shadow-premium group">
                 ...
                 <form onSubmit={handleSubmit} className="space-y-8">
                    <div className="space-y-3">
                      ...
                      <div className="relative">
                         <Phone size={16} className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-600" />
                         <input 
                            type="tel" 
                    ...
                    <div className="space-y-3">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">Reason for Blocking</label>
                      <input 
                         type="text" 
                         value={form.reason} 
                         onChange={e => setForm(f => ({ ...f, reason: e.target.value }))}
                         placeholder="e.g. Fake Orders"
                         className="w-full h-16 px-6 bg-white/5 border border-white/10 rounded-2xl text-sm font-bold text-white focus:bg-white/10 focus:border-rose-500 focus:outline-none transition-all placeholder:text-slate-700" 
                      />
                    </div>
           ...
           <div className="bg-slate-50 border border-slate-100 rounded-[3rem] p-10 space-y-6">
              <div className="flex items-center gap-3">
                 <Activity size={16} className="text-slate-400" />
                 <h4 className="text-[10px] font-black text-slate-950 uppercase tracking-widest">System Stats</h4>
              </div>
              <div className="grid grid-cols-2 gap-4">
                 <div className="p-6 bg-white rounded-2xl border border-slate-100">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Total Blocked</p>
                    <p className="text-3xl font-black text-rose-600 tracking-tighter">{spamList.length}</p>
                 </div>
                 <div className="p-6 bg-white rounded-2xl border border-slate-100">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Safety Rating</p>
           ...
           <div className="bg-white rounded-[3rem] border border-slate-100 shadow-premium overflow-hidden">
              <div className="px-10 py-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                 <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-[10px] font-black text-slate-950 uppercase tracking-widest">Restricted Identifier Registry</h3>
                 </div>
              ...
              {loading ? (
                <div className="px-10 py-24 text-center">
                   <div className="w-8 h-8 border-2 border-slate-200 border-t-rose-500 rounded-full animate-spin mx-auto mb-4" />
                ...
                <div className="px-10 py-32 text-center space-y-4">
                   <Shield size={48} className="mx-auto text-slate-100" strokeWidth={1} />
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Zero anomalies detected</p>
                </div>
              ) : (
                <div className="divide-y divide-slate-50">
                   {spamList.map((entry, i) => (
                     <motion.div 
                        ...
                        key={entry._id} 
                        className="flex items-center gap-6 px-10 py-8 group hover:bg-slate-50/50 transition-all duration-300"
                     >
                        ...
                        <div className="flex-1 min-w-0">
                           <p className="text-lg font-black text-slate-950 tracking-tighter mb-1 font-mono">{entry.phoneNumber}</p>
                           <div className="flex items-center gap-3">
                              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{entry.reason || 'Anomalous Activity'}</span>
                              <div className="w-1 h-1 bg-slate-200 rounded-full" />
                              <span className="text-[9px] font-bold text-slate-300 uppercase tracking-tighter">{new Date(entry.createdAt).toLocaleDateString()}</span>
                           </div>
                        </div>
                        <button 
                           onClick={() => handleDelete(entry._id, entry.phoneNumber)}
                           className="w-12 h-12 bg-slate-50 text-slate-300 rounded-xl flex items-center justify-center hover:bg-rose-50 hover:text-rose-600 transition-all group/btn"
                        >
              ...
              <div className="px-10 py-6 border-t border-slate-50 bg-slate-50/20">
                 <div className="flex items-center justify-between">
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Total Registry Entropy: {spamList.length}</p>

  ...
  return (
    <div className="space-y-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
           <h1 className="text-2xl font-bold text-slate-950 tracking-tight">Inventory</h1>
           <p className="text-slate-400 text-[13px] font-medium mt-1">Manage stock levels and track activity.</p>
        </div>
        
        <div className="flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
           <Database size={14} className="text-slate-400" />
           <span className="text-[11px] font-bold text-slate-950 uppercase tracking-widest">{products.length} Products</span>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-4">
           <div className="bg-slate-950 rounded-2xl p-8 shadow-xl shadow-slate-900/10">
              ...
              <form onSubmit={handleAdjust} className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">Product</label>
                  <select 
                  ...
                  >
                    ...
                    {products.map(p => <option key={p._id} value={p._id} className="bg-slate-900">{p.name} ({p.stock})</option>)}
                  </select>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">Reason</label>
                    <input 
                      type="text" 
                      value={adjustForm.reason} 
                      onChange={e => setAdjustForm(f => ({...f, reason: e.target.value}))}
                      placeholder="e.g. Restock"
                      className="w-full h-11 px-4 bg-white/5 border border-white/10 rounded-xl text-[13px] font-bold text-white focus:bg-white/10 outline-none transition-all placeholder:text-slate-600" 
                    />
                  </div>
                </div>

                <button type="submit" disabled={submitting} className="w-full h-12 bg-white text-slate-950 rounded-xl font-bold text-[11px] uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all flex items-center justify-center gap-2">
                  {submitting ? <div className="w-4 h-4 border-2 border-slate-950/30 border-t-slate-950 rounded-full animate-spin" /> : <TrendingUp size={14} />}
                  Apply
                </button>
              </form>
           </div>
           ...
           <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
              <div className="px-8 py-5 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
                 <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-[11px] font-bold text-slate-950 uppercase tracking-widest">Recent Activity</h3>
                 </div>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-50">
                      ...
                      <th className="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Change</th>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {logs.slice(0, 8).map((log) => (
                      <tr key={log._id} className="hover:bg-slate-50/30 transition-all">
                        <td className="px-8 py-3.5 text-[12px] font-bold text-slate-950">{log.product?.name || 'Deleted'}</td>
                        ...
                        <td className="px-8 py-3.5 text-right text-[10px] font-medium text-slate-400 uppercase">{new Date(log.createdAt).toLocaleDateString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
